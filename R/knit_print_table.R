...
style_id <- function(x, type, si) {
  si[
    si$style_type %in% type & si$style_name %in% x,
    "style_id"
  ]
}
validate_style <- function(x, type, si) {
  ...
  if (length(validated_style) != 1) {
    ...
    msg <- paste0(
      ...
      " style ",
      shQuote(x),
      ". Switching to default one named ",
      shQuote(validated_style),
      "."
    )
...
knit_print.data.frame <- function(x, ...) {
  ...
  if (grepl("docx", opts_knit$get("rmarkdown.pandoc.to"))) {
    ...
    pt <- prop_table(
      style = tab_props$style,
      layout = tab_props$table_layout,
    ...
    if (!inherits(ph, "location_str")) {
      stop(
        "ph should be a placeholder location; ",
