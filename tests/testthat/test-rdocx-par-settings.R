...
test_that("scanning settings", {
  ...
  spacing_node <- xml_child(
    node_body,
    "/w:p/w:pPr[w:pStyle[@w:val='Normal']]/w:spacing"
  )
...
test_that("change styles", {
  node_body <- get_docx_xml(docx_file)
  normal_node <- xml_child(
    node_body,
    "/w:p[2]/w:pPr[w:pStyle[@w:val='Normal']]"
  )
  expect_false(inherits(normal_node, "xml_missing"))
  normal_node <- xml_child(
    node_body,
    "/w:p[3]/w:pPr[w:pStyle[@w:val='Normal']]"
  )
  expect_false(inherits(normal_node, "xml_missing"))
})
