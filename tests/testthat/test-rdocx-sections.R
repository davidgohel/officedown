...
test_that("reading sections properties", {
  ...
  next_node <- xml_find_first(
    node_body,
    "w:p[w:pPrw/sectPr/w:type[@w:val='continuous']]"
  )
  next_node <- xml_find_first(
    node_body,
    "w:p[w:pPr/sectPr/w:type[@w:val='continuous']]"
  )
  next_node <- xml_find_first(
    node_body,
    "//w:p[w:pPr/w:sectPr/w:cols[@w:num='2']]/preceding-sibling::w:p[1]"
  )
  ...
  part_references <- grep(
    "(header|footer)Reference",
    xml_name(xml_children(pr_nodes)),
    value = TRUE
  )
